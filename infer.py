...
def parse_args():
    parse = argparse.ArgumentParser(description='X-Recon')
    ...
    parse.add_argument('--which_model_netG', type=str, default='', dest='gen_net',
                       help='cover the generator defined by yaml')
    parse.add_argument('--if_ema', type=str2bool, default='False', dest='arg_if_ema',
                       help='cover the if_ema defined by yaml')
...
if __name__ == '__main__':
    ...
    opt['which_model_netG'] = args.gen_net
    opt['if_ema'] = args.arg_if_ema
    print_easy_dict(opt)
    ...
    web_dir = os.path.join(opt.resultdir, opt.data, '%s_%s_%s' % (opt.tag, opt.dataset, opt.check_point))
    ...
    for idx, data in tqdm.tqdm(enumerate(dataloader)):
        ...
        if idx <= opt.how_many:
            ...
            # Image HTML
            #
            # save_images(webpage, visuals, img_path, gan_model.get_normalization_list(), max_image=50) # 临时不生成html
            #
            # CT Source
            #
            ...
            # # To [0, 1]
            generate_CT_transpose = tensor_back_to_unnormalization(generate_CT_transpose, opt.CT_MEAN_STD[0], opt.CT_MEAN_STD[1])
            real_CT_transpose = tensor_back_to_unnormalization(real_CT_transpose, opt.CT_MEAN_STD[0], opt.CT_MEAN_STD[1])

            # Clip generate_CT # 这个逻辑之处在于，本身是0-1，先剪裁，然后算指标，最后还原回CT值
