    ...
    def print_networks(self, verbose):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                num_params = 0
                if net.__class__.__name__ == 'ExponentialMovingAverage':
                    for param in net.shadow_params:
    ...
    def save_networks(self, which_epoch, total_steps, latest=False):
        for name in self.model_names:
            if isinstance(name, str):
                ...
                elif len(self.gpu_ids) > 0 and torch.cuda.is_available():
    ...
    def load_networks(self, which_epoch, load_Path=None, latest=False):
        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                print('**loading {} parameters from {}(saved model), net size = {}'.format(len(will_state_dict),
                                                                                           len(state_dict),
                                                                                           len(net.state_dict())))
