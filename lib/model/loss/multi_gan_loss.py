...
# Weighted Restruction Loss: only implement personally defined weighted mse, not used now
class WeightedRestructionLoss(nn.Module):
    '''
    reduction: 'elementwise_mean' or 'none'
    '''

    def __init__(self, distance='l1', reduction='elementwise_mean'):
        super(WeightedRestructionLoss, self).__init__()
    ...
    def forward(self, gt, pred, weight):
        if self.distance == 'mse' and self.reduction == 'elementwise_mean':
