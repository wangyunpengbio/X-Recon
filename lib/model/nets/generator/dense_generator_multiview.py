...
from lib.model.nets.utils import CoordConv2D, CoordConv3D
...
def UNetLike_DownStep5(input_shape, encoder_input_channels, decoder_output_channels, decoder_out_activation,
    ...
    encoder_block_list = [4, 8, 16, 8, 4] # 每个encoder_layer里面名为_DenseLayer2D的block数目
    # decoder_block_list = [1, 1, 1, 1, 1, 0]
    growth_rate = 32
    encoder_channel_list = [64]
    decoder_channel_list = [16, 16, 32, 64, 128, 256]
    # decoder_channel_list = [8, 8, 16, 32, 64, 128]
...
# 在小网络中加入CoordConv
def UNetLike_DownStep5_CoordConv(input_shape, encoder_input_channels, decoder_output_channels, decoder_out_activation,
                       encoder_norm_layer, decoder_norm_layer, upsample_mode, decoder_feature_out=False):
    # 64, 32, 16, 8, 4
    encoder_block_list = [6, 12, 24, 16, 6]
    decoder_block_list = [1, 2, 2, 2, 2, 0]
    ...
    decoder_channel_list = [16, 16, 32, 64, 128, 256]
    decoder_begin_size = input_shape // pow(2, len(encoder_block_list))
    return UNetLike_DenseDimensionNet_CoordConv(encoder_input_channels, decoder_output_channels, decoder_begin_size,
                                      encoder_block_list, decoder_block_list, growth_rate, encoder_channel_list,
                                      decoder_channel_list, decoder_out_activation, encoder_norm_layer,
                                      decoder_norm_layer, upsample_mode, decoder_feature_out)
...
# 这个定义的是小网络，增加了CoordConv
class UNetLike_DenseDimensionNet_CoordConv(nn.Module):
    def __init__(self, encoder_input_channels, decoder_output_channels, decoder_begin_size, encoder_block_list,
        ...
        super(UNetLike_DenseDimensionNet_CoordConv, self).__init__()
        ...
        encoder_layers0 = [
            ...
            CoordConv2D(encoder_input_channels, encoder_channel_list[0], kernel_size=7, padding=0, bias=use_bias),
        ...
        for index, channel in enumerate(encoder_block_list):
            # encoder里面，每次都把Dense_2DBlock组装进去，然后再用1*1的卷积层做末尾，每次末尾1*1卷积会将channel数减半
            # pooling
            down_layers = [
                encoder_norm_layer(num_input_channels),
                activation,
                CoordConv2D(num_input_channels, num_input_channels, kernel_size=3, stride=2, padding=1, bias=use_bias),

            ]
            down_layers += [
                Dense_2DBlock(encoder_block_list[index], num_input_channels, bn_size, growth_rate, encoder_norm_layer,
                              activation, use_bias),
            ]
            ...
            # 中间穿插了1*1的卷积层
                ...
                down_layers += [
                    encoder_norm_layer(num_input_channels),
                    activation,
                    CoordConv2D(num_input_channels, num_out_channels, kernel_size=1, stride=1, padding=0, bias=use_bias),
                ]
        ...
        for index, channel in enumerate(decoder_channel_list[:-1]):
            ...
            if index != (len(decoder_channel_list) - 2):
                decoder_compress_layers += [
                    CoordConv3D(in_channels * 2, in_channels, kernel_size=3, padding=1, bias=use_bias),
                    decoder_norm_layer(in_channels),
                    activation
                ]
                for _ in range(decoder_block_list[index + 1]):
                    decoder_layers += [
                        CoordConv3D(in_channels, in_channels, kernel_size=3, padding=1, bias=use_bias),
                        decoder_norm_layer(in_channels),
                        activation
                    ]
            decoder_layers += [
        ...
        decoder_layers = []
        decoder_compress_layers = [
            CoordConv3D(decoder_channel_list[0] * 2, decoder_channel_list[0], kernel_size=3, padding=1, bias=use_bias),
            decoder_norm_layer(decoder_channel_list[0]),
            activation
        ]
        for _ in range(decoder_block_list[0]):
            decoder_layers += [
                CoordConv3D(decoder_channel_list[0], decoder_channel_list[0], kernel_size=3, padding=1, bias=use_bias),
                decoder_norm_layer(decoder_channel_list[0]),
                activation
            ]
        if decoder_feature_out:
        ...
        self.decoder_layer = nn.Sequential(*[
            CoordConv3D(decoder_channel_list[0], decoder_output_channels, kernel_size=7, padding=3, bias=use_bias),
...
# 这个定义的是大网络，增加了CoordConv
class MultiView_UNetLike_DenseDimensionNet_CoordConv(nn.Module):
    def __init__(self, view1Model, view2Model, view1Order, view2Order, backToSub, decoder_output_channels,
        ...
        super(MultiView_UNetLike_DenseDimensionNet_CoordConv, self).__init__()
        ...
        decoder_layers = []
        decoder_compress_layers = [
            CoordConv3D(self.decoder_channel_list[0] * 2, self.decoder_channel_list[0], kernel_size=3, padding=1,
                      bias=use_bias),
        ...
        for _ in range(self.decoder_block_list[0]):
            decoder_layers += [
                CoordConv3D(self.decoder_channel_list[0], self.decoder_channel_list[0], kernel_size=3, padding=1,
                          bias=use_bias),
                decoder_norm_layer(self.decoder_channel_list[0]),
                activation
            ]
        setattr(self, 'decoder_layer' + str(-1), nn.Sequential(*(decoder_compress_layers + decoder_layers)))
        self.decoder_layer = nn.Sequential(*[
            CoordConv3D(self.decoder_channel_list[0], decoder_output_channels, kernel_size=7, padding=3, bias=use_bias),
            decoder_out_activation()
        ])
    ...
    def forward(self, input):
        ...
        for i in range(self.n_downsampling - 1, -2, -1):
            if i == (self.n_downsampling - 1):
                view1_next_input = getattr(self.view1Model, 'decoder_compress_layer' + str(i))(view1_next_input) # 没东西
                view2_next_input = getattr(self.view2Model, 'decoder_compress_layer' + str(i))(view2_next_input) # 没东西
                ########### MultiView Fusion
        ...
        return self.view1Model.decoder_layer(view1_next_input), self.view2Model.decoder_layer(
            view2_next_input), self.decoder_layer(view_next_input)
...
# 这个定义的是大网络，增加了多个scale的toHU，使用Skip connect再add
class MultiView_UNetLike_DenseDimensionNet_SkipConnect(nn.Module):
    def __init__(self, view1Model, view2Model, view1Order, view2Order, backToSub, decoder_output_channels,
                 decoder_out_activation, decoder_block_list=None, decoder_norm_layer=nn.BatchNorm3d,
                 upsample_mode='nearest'):
        super(MultiView_UNetLike_DenseDimensionNet_SkipConnect, self).__init__()
        self.view1Model = view1Model
        self.view2Model = view2Model
        self.view1Order = view1Order
        self.view2Order = view2Order
        ...
        self.n_downsampling = view2Model.n_downsampling
        self.decoder_channel_list = view2Model.decoder_channel_list
        # self.upsampling_HU = nn.Upsample(scale_factor=2, mode='nearest') # HU里面的upsample_mode == 'trilinear' or upsample_mode == 'nearest'
        self.upsampling_HU = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False) # HU里面的upsample_mode == 'trilinear' or upsample_mode == 'nearest'，bilinear是4D的input，不可行
        if decoder_block_list is None:
            self.decoder_block_list = view2Model.decoder_block_list
        else:
            self.decoder_block_list = decoder_block_list
        ...
        for index, channel in enumerate(self.decoder_channel_list[:-1]):
            ...
            decoder_layers = []
            decoder_compress_layers = []
            if index != (len(self.decoder_channel_list) - 2): # 除了最中心的，只进行上采样，其他的都要增加block
                decoder_compress_layers += [
                    nn.Conv3d(in_channels * 2, in_channels, kernel_size=3, padding=1, bias=use_bias),
                    decoder_norm_layer(in_channels),
                    activation
                ...
                # 在第二次上采样之前进行toHU
                setattr(self, 'tohu_layer' + str(index), nn.Sequential(*[
                    nn.Conv3d(in_channels * 2, decoder_output_channels, kernel_size=1, padding=0, bias=use_bias),
                    decoder_out_activation()
                ]))
            decoder_layers += [
                Upsample_3DUnit(3, in_channels, out_channels, decoder_norm_layer, scale_factor=2,
        ...
        setattr(self, 'decoder_layer' + str(-1), nn.Sequential(*(decoder_compress_layers + decoder_layers)))
        setattr(self, 'tohu_layer' + str(-1), nn.Sequential(*[
                    nn.Conv3d(self.decoder_channel_list[0] * 2, decoder_output_channels, kernel_size=1, padding=0, bias=use_bias),
                    decoder_out_activation()
        ]))

        # cat到一起去再输出
        # self.decoder_layer = nn.Sequential(*[
        #     nn.Conv3d(self.n_downsampling, decoder_output_channels, kernel_size=7, padding=3, bias=use_bias),
        #     decoder_out_activation()
        # ])
    ...
    def forward(self, input):
        ...
        # View 1 and 2 decoding process Part2
        for i in range(self.n_downsampling - 1, -2, -1):
            if i == (self.n_downsampling - 1): #该判断控制了子网络是否需要feature_linker再cat过来
                view1_next_input = getattr(self.view1Model, 'decoder_compress_layer' + str(i))(view1_next_input) # 没东西
                view2_next_input = getattr(self.view2Model, 'decoder_compress_layer' + str(i))(view2_next_input) # 没东西
                ########### MultiView Fusion
                # Method One: Fused feature back to sub-branch
                ...
                if self.backToSub:
                    ...
                    view1_next_input = view_avg.permute(*self.view1Order)
                    view2_next_input = view_avg.permute(*self.view2Order)
                    # 放入主网络的decoder之前cat到一起去，同时塞进HU
                    # view_next_HU = getattr(self, 'tohu_layer' + str(i))(torch.cat((view_avg, view_next_input), dim=1))
                    # if view_final_HU == None:
                    #     view_final_HU = view_next_HU
                    # else:
                    #     view_final_HU = self.upsampling_HU(view_final_HU)
                    #     view_final_HU = torch.cat((view_final_HU, view_next_HU), dim=1)
                    view_next_HU = getattr(self, 'tohu_layer' + str(i))(torch.cat((view_avg, view_next_input), dim=1))
                    if view_final_HU is None:
                        view_final_HU = view_next_HU / 2 # TO 这里直接除以2就是加权模式
                    else:
                        view_final_HU = self.upsampling_HU(view_final_HU)
                        view_final_HU = (view_final_HU + view_next_HU) / 2 # TO 这里直接除以2就是加权模式
                    view_next_input = getattr(self, 'decoder_layer' + str(i))(
                        torch.cat((view_avg, view_next_input), dim=1))
                # Method Two: Fused feature only used in main-branch
                else:
        ...
        return self.view1Model.decoder_layer(view1_next_input), self.view2Model.decoder_layer(
            view2_next_input), self.decoder_layer(view_final_HU) # view_final_HU #self.decoder_layer(view_next_input)
