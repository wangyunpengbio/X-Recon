...
def tensor_to_image(tensor, imtype=np.uint8):
    ...
    if np.uint8 == imtype:
        if np.max(img) > 1:
            img = post_process_to_range_01(img)
            # print(np.max(img))
...
def post_process_to_range_01(img_np):
    min_val = img_np.min()
    max_val = img_np.max()
    img_result = (img_np - min_val) / (max_val - min_val)
    return img_result
...
def save_images(webpage, visuals, image_path, normalize_list, aspect_ratio=1.0, width=256, max_image=10):
    ...
    for label, image_tensor in visuals.items():
        ...
        image_list = [tensor_back_to_unnormalization(img, normalize_list[count][0], normalize_list[count][1]) for img in
                      image_list]
        ims, txts, links = [], [], []
        count += 1
...
class Visualizer(object):
    ...
    def add_image(self, name, image_dict, normalize_list, step, max_image=10):
        ...
        for key, image in image_dict.items():
            ...
            image_list = [tensor_back_to_unnormalization(img, normalize_list[count][0], normalize_list[count][1]) for
                          img in image_list]
...
def main():
    ...
    test_file = r'/home/u18111510027/X2CT/3DGAN/data/LIDC-HDF5-256/LIDC-IDRI-0001.20000101.3000566.1/ct_xray_data.h5'
    import h5py
    ...
    visual.add_image('a', visual_dict, 1, 1)  # 此处没有提供normalize_list，如果需要运行，需要把原函数中对应转换的行注释掉
